...
| `WORKSPACE_MCP_PORT` | | Listening port — default `8000`. In stdio mode, leave **unset** to let the OAuth callback use a fresh ephemeral port per auth flow (recommended for Desktop/PKCE clients; lets multiple concurrent stdio sessions coexist without contending for one fixed port). Set it only to pin a fixed callback port for a Web/confidential client whose redirect URI is registered. The `PORT` env var takes precedence if set. |
...
- **OAuth Callback**: Uses `http://localhost:8000/oauth2callback` for development (requires `OAUTHLIB_INSECURE_TRANSPORT=1`). If you use a **web/confidential OAuth client** (not the recommended Desktop client), set `WORKSPACE_MCP_PORT` to the port whose redirect URI you registered in Google Cloud Console (e.g. `export WORKSPACE_MCP_PORT=8123` ↔ `http://localhost:8123/oauth2callback`); Desktop and PKCE clients do not require this.
- **Transport-Aware Callbacks**: Stdio mode starts a minimal HTTP server only for OAuth. When `WORKSPACE_MCP_PORT` is unset (Desktop/PKCE clients, which accept any loopback port), each auth flow binds a fresh ephemeral port and releases it as soon as the callback resolves (or a 5-minute timeout), so concurrent stdio sessions never contend for a single fixed port. Pinning `WORKSPACE_MCP_PORT` keeps a fixed callback port for Web/confidential clients.
  - *Across sessions (inter-session):* each stdio process binds its own OS-assigned ephemeral port, so two sessions authenticating at once get two distinct ports and never collide — even if one user abandons a flow (closes the browser without authenticating); that session's port is reclaimed by its own timeout and never blocks the other.
  - *Within one session (intra-session):* a session runs at most one callback listener at a time. Re-triggering auth before the prior flow's timeout **tears down the old listener (freeing its port), binds a new ephemeral port, and arms a fresh timeout** — it does not reuse the old port, reset its timer in place, or listen on two ports in parallel. The new authorization URL points at the new port, so the freshly opened browser hits the live listener; the superseded flow is abandoned and its port already freed.
  - *Abandoned flows:* a tab/browser closed without authenticating produces no callback (the OAuth page is Google's origin and cannot signal us), so the port is held until the 5-minute watchdog fires and releases it. The callback-success and watchdog paths are the only two release triggers.
